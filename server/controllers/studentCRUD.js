...
export const createStudent = async (req, res) => {
  ...
  try {
    ...
    if (preStudent) {
      ...
      const studentRegister = new Student(req.body);
  ...
  try {
    ...
    if (preStudent) {
      res
        .status(400)
        .json({
          message: "Roll Number is already assigned to another student",
        });
    } else {
      const update = {
        rollNumber,
    ...
    res
      .status(400)
      .json({ message: "Please try again later", error: error.message });
  }
};
...
export const deleteStudent = async (req, res) => {
  const { _id } = req.body;
  try {
    const deleted = await Student.findOneAndDelete({ _id: _id });
